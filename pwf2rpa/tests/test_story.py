...
class FullPipelineStoryGateTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        script = Path(__file__).resolve().parents[2] / "skills" / "total-pipe-deck" / "scripts" / "rpa_full_pipeline.py"
        spec = importlib.util.spec_from_file_location("rpa_full_pipeline", script)
        cls.pipeline = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(cls.pipeline)

    def test_gate_accepts_story_briefs(self):
        payload, _warnings = convert(make_workflow(), story=make_story(), strict_fit=False)
        ok, note = self.pipeline.validate_story_provenance(payload)
        self.assertTrue(ok, note)

    def test_gate_rejects_legacy_briefs(self):
        ok, note = self.pipeline.validate_story_provenance({"slide_briefs": [{"title": "legacy"}]})
        self.assertFalse(ok)
        self.assertIn("story_reasoning", note)
